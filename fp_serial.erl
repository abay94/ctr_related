...
init(Owner,Cmd)->
  ...
  receive
    {Port, {data, <<"OK">>}}->
      Owner!{self(),ok},
      loop(Port,Owner,<<>>);
    {Port, {data, <<"ERROR: ",Error/binary>>}}->Owner!{self(),{error,Error}}
  after
...
loop(Port,Owner,Buf)->
  receive
    {Port, {exit_status, Status}} ->
      erlang:error({port_error,Status});
    {Owner,send,Data}->
...
create_program_file(PortName)->
  Dir=code:priv_dir(fp)++"/serial/",
  Source=
  case os:type() of
    {unix, linux}->"fp_serial";
    {win32, _}->"fp_serial.exe"
  end,
  SourcePath=Dir++Source,
  case filelib:is_file(SourcePath) of true->ok; false->erlang:error({file_not_found,SourcePath}) end,
  case os:type() of
    ...
    {win32, _}->
      ...
      case filelib:is_file(ResultPath) of
        true->ok;
        false->
          case file:copy(SourcePath, ResultPath) of {error,Error}->erlang:error(Error); _->ok  end
      end,
      ResultPath
  end.
